...
export function generateResolvers(
  ...
  const { generateResolverMap, mandatoryRootOperationTypes = false } = options;
  ...
  statements.push(
    ...resolverTypes.map((type) =>
      createResolversForType(context, type, mandatoryRootOperationTypes),
    ),
  );

  const extra: ts.Statement[] = [];
  if (generateResolverMap) {
    extra.push(createResolversMap(resolverTypes, mandatoryRootOperationTypes));
  }
...
function createResolversForType(
  ...
  switch (kind) {
    case "OBJECT": {
      return createObjectTypeResolvers(
        context,
        type,
  ...
  const resolversObject = factory.createInterfaceDeclaration(
    ...
    type.fields.map((field) =>
      factory.createPropertySignature(
        ...
        field.name,
        mandatoryRootOperationTypes && isRootOperationType(type.name)
          ? undefined
  ...
  return factory.createInterfaceDeclaration(
    ...
    types.map(({ name }) => {
      return factory.createPropertySignature(
        [factory.createModifier(ts.SyntaxKind.ReadonlyKeyword)],
        name,
        mandatoryRootOperationTypes && isRootOperationType(name)
          ? undefined
          : factory.createToken(ts.SyntaxKind.QuestionToken),
