...
The executor always returns raw incremental payloads. It does not delay the initial result or merge deferred
payloads into it; those are transport delivery policies. Deferred fragments can still be started early with:
...
When multiple patches resolve in the same microtask checkpoint, they are emitted in one subsequent result.
This produces a single payload for concurrently resolved list items without introducing a timeout. Transports
can retain the initial result and consume subsequent results according to their own merge, serialization,
and delivery policy.
