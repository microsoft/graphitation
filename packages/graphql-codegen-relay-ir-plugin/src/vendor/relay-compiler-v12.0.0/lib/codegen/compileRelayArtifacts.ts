...
import RelayCodeGenerator from "relay-compiler/lib/codegen/RelayCodeGenerator";
import * as InlineFragmentsTransform from "relay-compiler/lib/transforms/InlineFragmentsTransform";

// const filterContextForNode = require('../core/filterContextForNode');

import type CompilerContext from "relay-compiler/lib/core/CompilerContext";
...
import type { Reporter } from "relay-compiler/lib/reporters/Reporter";
...
//   ...
// };
type RelayCompilerTransforms = typeof import("relay-compiler/lib/core/RelayIRTransforms");
...
function compile(
  ...
  const results: Array<[GeneratedDefinition, GeneratedNode]> = [];
  const schema = context.getSchema();

  const operationContext = fragmentContext.applyTransform(
  ...
  for (const node of codeGenContext.documents()) {
    if (node.kind === "Root") {
      const fragment = operationContext.getRoot(node.name);
      const request: Request = {
        ...
        text: JSON.stringify(fragment), // NOTE: Presumably this is cheaper than printing the doc
      };
      // NOTE: Nullify the text property after generating the code, so it doesn't end up in the artifacts.
      //   results.push([request, RelayCodeGenerator.generate(schema, request)]);
      const generatedNode = RelayCodeGenerator.generate(schema, request);
      request.text = null;
      (generatedNode.params as any).text = null;
      results.push([request, generatedNode]);
    } else {
      results.push([
        node,
        RelayCodeGenerator.generate(schema, node as Fragment),
      ]);
    }
  }
...
function compileRelayArtifacts(
  ...
  const fragmentContext = createFragmentContext(context, transforms, reporter);
...
export default compileRelayArtifacts;
