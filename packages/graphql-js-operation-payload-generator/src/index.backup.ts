  ...
  const visitor: VisitorWithMockData = {
    ...
    SelectionSet: {
      ...
      enter(selectionSetNode, _key, parent) {
        if (Array.isArray((parent as any).userMockData)) {
          console.log((parent as any).userMockData);
          // TODO visit selectionSetNode as many times as needed to fill array?
        }
...
function mockCompositeType(
  ...
  const data = resolveValue(type.name, context, plural, undefined) as MockData;
