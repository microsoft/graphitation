...
describe("useQuery Hook", () => {
  describe("General use", () => {
    ...
    it("useQuery result is referentially stable", async () => {
      ...
      const { result, waitFor, rerender } = renderHook(() => useQuery(query), {
        wrapper,
      });
    ...
    const expectFrames = <TData, TVariables>(
      result: RenderResult<QueryResult<TData, TVariables>>,
      expectedPartialFrames: Partial<QueryResult<TData, TVariables>>[],
    ) => {
      const actualPartialFrames = result.all.map((actualFrame, i) => {
        ...
        if (expectedPartialFrame) {
          const actualPartialFrame: Partial<
            Record<keyof QueryResult<TData, TVariables>, any>
          > = {};
          (
            Object.keys(
              expectedPartialFrame,
            ) as (keyof typeof expectedPartialFrame)[]
          ).forEach((key) => {
    ...
    itAsync(
      "should not error when forcing an update with React >= 16.13.0",
      (resolve, reject) => {
            ...
            observer.next({
              data: {
                people: (gender === "all"
                  ...
                  : peopleData
                ).map(({ gender, ...person }) => person), // ForestRun: result should conform to query
              },
            });
            observer.complete();
          });
