...
describe('Combining client and server state/operations', () => {
  ...
  // ForestRun FIXME: snapshots inside watchQuery callback are not compared (most likely watch is not triggered)
  itAsync.skip('should handle a simple query with both server and client fields', (resolve, reject) => {
    const query = gql`
      query GetCount {
        count @client
        lastCount
  ...
  // ForestRun FIXME: snapshots inside watchQuery callback are not compared (most likely watch is not triggered)
  itAsync.skip('should support nested querying of both server and client fields', (resolve, reject) => {
    const query = gql`
      query GetUser {
        user {
          firstName @client
