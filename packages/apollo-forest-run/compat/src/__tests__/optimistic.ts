...
describe('optimistic mutation results', () => {
  ...
  describe('optimistic updates using `update`', () => {
    // ForestRun doesn't cache mutations by default
    const mutation = gql`
      mutation createTodo @cache {
        # skipping arguments in the test since they don't matter
        createTodo {
          id
          text
    ...
    itAsync("final update ignores optimistic data", (resolve, reject) => {
      ...
      // ForestRun doesn't cache mutation results by default
      const mutation = gql`
        mutation AddItem($item: Item!) @cache {
          addItem(item: $item) {
            text
          }
        }
