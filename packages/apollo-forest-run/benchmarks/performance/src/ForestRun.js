...
const getRef = (ref) => REFS_POOL.get(ref) ?? { __ref: ref };
class ForestRun extends client_1.ApolloCache {
        ...
        if (fieldName) {
            ...
            const storeFieldName = (0, keys_1.fieldToStringKey)(argValues && firstChunk.type
                ? {
                    name: fieldName,
                    args: argValues,
                    keyArgs: this.env.keyArgs?.(firstChunk.type, fieldName, argValues),
                }
                : fieldName);
            return this.modify({
                id: id,
    ...
    collectAffectedOperationsAndNodes(transaction) {
        transaction.affectedOperations ?? (transaction.affectedOperations = new Set());
        transaction.affectedNodes ?? (transaction.affectedNodes = new Set());
        for (const entry of transaction.changelog) {
            ...
            if (this.newWatches.size) {
                transaction.watchesToNotify ?? (transaction.watchesToNotify = new Set());
        ...
        if (optimistic) {
            for (const layer of this.store.optimisticLayers) {
                for (const [id, optimisticTree] of layer.trees.entries()) {
                    ...
                    output[key(optimisticTree.operation)] = stableConvert(optimisticTree.operation, tree?.result.data ?? null, optimisticTree.result.data);
                }
            }
        }
        return output;
    ...
    runTransaction(options) {
        ...
        if (parentTransaction) {
            parentTransaction.watchesToNotify ?? (parentTransaction.watchesToNotify = new Set());
    ...
    removeOptimisticLayers(layerTag) {
        const activeTransaction = peek(this.transactionStack);
        (0, assert_1.assert)(activeTransaction);
        activeTransaction.affectedOperations ?? (activeTransaction.affectedOperations = new Set());
