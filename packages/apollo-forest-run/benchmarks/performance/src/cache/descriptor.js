...
function resolveOperationDescriptor(env, { operations, dataForest }, doc, variables, rootNodeKey) {
    ...
    const variablesWithDefaultValues = (0, operation_1.applyDefaultValues)(variables ?? {}, documentDescriptor.definition.variableDefinitions);
    ...
    if (!match) {
        ...
        match = (0, operation_1.describeOperation)(env, documentDescriptor, resultTreeDescriptor, variables ?? {}, variablesWithDefaultValues, variablesKey, rootTypeName, rootNodeKey);
...
function getDiffDescriptor(env, store, options) {
    ...
    if (!operationDescriptor) {
        operationDescriptor = resolveOperationDescriptor(env, store, options.query, options.variables, options.rootId ?? options.id);
...
function resolveResultDescriptor(env, store, operation) {
    ...
    (0, assert_1.assert)(byDocument?.size); // at least operation itself is expected to be there
