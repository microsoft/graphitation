...
function diffObject(base, model, env, state = { difference: undefined }) {
    ...
    if ((_a = context.errors) === null || _a === void 0 ? void 0 : _a.length) {
...
function diffValue(env, base, model, state) {
    ...
    if (Difference.isObjectDifference(difference) ||
        ...
        difference.errors = Value.isMissingValue(model)
            ? [{ kind: types_1.DiffErrorKind.MissingModelValue }, ...((_a = context.errors) !== null && _a !== void 0 ? _a : [])]
            : context.errors;
    }
    return difference;
}
...
function diffObjectChunk(context, base, model, diff) {
    ...
    const fieldQueue = (_a = Difference.getFieldQueue(diff)) !== null && _a !== void 0 ? _a : Value.aggregateFieldNames(model);
    for (const fieldName of fieldQueue) {
        ...
        const fieldEntries = (_b = Difference.getFieldEntryQueue(diff, fieldName)) !== null && _b !== void 0 ? _b : Value.aggregateFieldEntries(model, fieldName);
...
function diffFieldEntry(context, base, model, fieldEntry, parentObjectDiff) {
    ...
    const valueDifference = Value.isMissingValue(baseValue) || Value.isMissingValue(modelValue)
        ? diffMissingFieldValues(context, base, model, fieldEntry, baseValue, modelValue, parentObjectDiff, currentDiff === null || currentDiff === void 0 ? void 0 : currentDiff.state)
        : diffValueInternal(context, baseValue, modelValue, currentDiff === null || currentDiff === void 0 ? void 0 : currentDiff.state);
    if (!valueDifference) {
        return parentObjectDiff;
    }
    parentObjectDiff = parentObjectDiff !== null && parentObjectDiff !== void 0 ? parentObjectDiff : Difference.createObjectDifference();
...
function diffCompositeListValue(context, base, model, diff) {
    ...
    const layoutDiffResult = (_a = diff === null || diff === void 0 ? void 0 : diff.layout) !== null && _a !== void 0 ? _a : diffCompositeListLayout(context, base, model);
    if (layoutDiffResult === "BREAK") {
        // Fast-path, no further diffing necessary
        return;
    }
    const itemQueue = (_b = diff === null || diff === void 0 ? void 0 : diff.itemQueue) !== null && _b !== void 0 ? _b : model.data.keys();
    if (layoutDiffResult) {
        diff = diff !== null && diff !== void 0 ? diff : Difference.createCompositeListDifference();
    ...
    for (const index of itemQueue) {
        ...
        const itemDiff = diffValueInternal(context, baseItemValue, modelItemValue, (_c = diff === null || diff === void 0 ? void 0 : diff.itemState) === null || _c === void 0 ? void 0 : _c.get(index));
        if (itemDiff) {
            diff = diff !== null && diff !== void 0 ? diff : Difference.createCompositeListDifference();
...
function resolveItemKey(env, listChunk, index) {
    ...
    return key !== null && key !== void 0 ? key : false;
}
...
function shouldSkipChunkField(base, fieldEntry) {
    ...
    return matchingEntries === null || matchingEntries === void 0 ? void 0 : matchingEntries.every((field) => { var _a; return (_a = base.selection.skippedFields) === null || _a === void 0 ? void 0 : _a.has(field); });
}
function addMissingChunkFieldError(context, chunk, field, isModelChunk) {
    ...
    const error = existing !== null && existing !== void 0 ? existing : { kind, chunk, missingFields: [] };
    if (!existing) {
