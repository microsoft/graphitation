...
function collectSubFields(context, field) {
    ...
    if (!((_a = usages === null || usages === void 0 ? void 0 : usages[0].node.selectionSet) === null || _a === void 0 ? void 0 : _a.selections.length)) {
        return undefined;
    }
    const commonSelection = createEmptySelection();
    const possibleSelections = new Map([
...
function collectFields(context, selectionsByType, selectionSet, ancestorStack = [], typeCondition = null, fragmentAlias) {
    ...
    for (const node of selectionSet.selections) {
        ...
        if (node.kind === "Field") {
            ...
            collectFields(context, selectionsByType, node.selectionSet, ancestorStack, (_a = getTypeName(node)) !== null && _a !== void 0 ? _a : typeCondition);
            ancestorStack.pop();
        }
...
function completeSelections(context, possibleSelections, depth = 0) {
    ...
    for (const selection of possibleSelections.values()) {
        ...
        for (const fieldAliases of selection.fields.values()) {
            for (const fieldInfo of fieldAliases) {
                if ((_a = fieldInfo.args) === null || _a === void 0 ? void 0 : _a.size) {
                    (_b = selection.fieldsToNormalize) !== null && _b !== void 0 ? _b : (selection.fieldsToNormalize = []);
                    selection.fieldsToNormalize.push(fieldInfo);
                }
                if (fieldInfo.selection) {
                    (_c = selection.fieldsWithSelections) !== null && _c !== void 0 ? _c : (selection.fieldsWithSelections = []);
                ...
                for (const { node, ancestors } of fieldInfo.__refs) {
                    if (((_d = node.directives) === null || _d === void 0 ? void 0 : _d.length) ||
                        ancestors.some((ancestor) => { var _a; return (_a = ancestor.directives) === null || _a === void 0 ? void 0 : _a.length; })) {
                        (_e = selection.fieldsWithDirectives) !== null && _e !== void 0 ? _e : (selection.fieldsWithDirectives = []);
                        if (!selection.fieldsWithDirectives.includes(fieldInfo)) {
                            selection.fieldsWithDirectives.push(fieldInfo);
                        }
                        if (!((_f = selection.fieldsToNormalize) === null || _f === void 0 ? void 0 : _f.includes(fieldInfo))) {
                            (_g = selection.fieldsToNormalize) !== null && _g !== void 0 ? _g : (selection.fieldsToNormalize = []);
                            selection.fieldsToNormalize.push(fieldInfo);
                        }
                    }
        ...
        for (const spreadAliases of (_j = (_h = selection.spreads) === null || _h === void 0 ? void 0 : _h.values()) !== null && _j !== void 0 ? _j : EMPTY_ARRAY) {
            for (const spreadInfo of spreadAliases) {
                for (const { node, ancestors } of spreadInfo.__refs) {
                    if (((_k = node.directives) === null || _k === void 0 ? void 0 : _k.length) ||
                        ancestors.some((ancestor) => { var _a; return (_a = ancestor.directives) === null || _a === void 0 ? void 0 : _a.length; })) {
                        (_l = selection.spreadsWithDirectives) !== null && _l !== void 0 ? _l : (selection.spreadsWithDirectives = []);
...
function inferPossibleType(context, abstractType, possibleType) {
    ...
    if (!abstractType ||
        ...
        ((_a = context.inferredPossibleTypes[abstractType]) === null || _a === void 0 ? void 0 : _a.includes(possibleType))) {
        return;
    }
    (_b = (_c = context.inferredPossibleTypes)[abstractType]) !== null && _b !== void 0 ? _b : (_c[abstractType] = []);
...
function collectConcreteTypes(possibleTypes, type, acc = []) {
    ...
    const concreteTypes = (_a = possibleTypes[type]) !== null && _a !== void 0 ? _a : [];
...
function mergeSelectionsImpl(context, target, source) {
    ...
    if ((_a = source.spreads) === null || _a === void 0 ? void 0 : _a.size) {
        (_b = mutableTarget.spreads) !== null && _b !== void 0 ? _b : (mutableTarget.spreads = new Map());
...
function isAbstractType(context, typeName) {
    var _a;
    return Boolean(typeName && ((_a = context.possibleTypes) === null || _a === void 0 ? void 0 : _a[typeName]));
}
function addFieldEntry(context, fieldMap, node, ancestors) {
    const fieldAliases = fieldMap.get(node.name.value);
    let fieldGroup = fieldAliases === null || fieldAliases === void 0 ? void 0 : fieldAliases.find((field) => { var _a; return field.alias === ((_a = node.alias) === null || _a === void 0 ? void 0 : _a.value); });
    if (!fieldGroup) {
...
function addSpreadEntry(context, selectionsByType, fragment, node, ancestors) {
    ...
    let spreadGroup = spreadAliases === null || spreadAliases === void 0 ? void 0 : spreadAliases.find((spread) => spread.alias === alias);
    if (!spreadGroup) {
        spreadGroup = {
            name: node.name.value,
            alias,
...
function copyFieldInfo(context, info) {
    ...
    const copy = {
        ...
        watchBoundaries: info.watchBoundaries === EMPTY_ARRAY ||
            ...
            : [...info.watchBoundaries],
        __refs: [...((_a = info.__refs) !== null && _a !== void 0 ? _a : [])],
    };
    if (info.alias) {
...
function getFragmentAlias(node) {
    ...
    return ((_a = aliasAs === null || aliasAs === void 0 ? void 0 : aliasAs.value) === null || _a === void 0 ? void 0 : _a.kind) === "StringValue"
        ? aliasAs.value.value
        : undefined;
}
function shouldSkip(node) {
    ...
    if (isVariableNode(skipIf === null || skipIf === void 0 ? void 0 : skipIf.value) || isVariableNode(includeIf === null || includeIf === void 0 ? void 0 : includeIf.value)) {
        return false;
    }
...
function findDirective(node, name) {
    var _a;
    return (_a = node.directives) === null || _a === void 0 ? void 0 : _a.find((directive) => directive.name.value === name);
}
function findArgument(node, name) {
    var _a;
    return (_a = node === null || node === void 0 ? void 0 : node.arguments) === null || _a === void 0 ? void 0 : _a.find((arg) => arg.name.value === name);
}
function isVariableNode(value) {
    return (value === null || value === void 0 ? void 0 : value.kind) === "Variable";
}
...
const isPossibleWatchBoundary = (node) => { var _a, _b; return (_b = (_a = node.directives) === null || _a === void 0 ? void 0 : _a.some((d) => d.name.value === "nonreactive")) !== null && _b !== void 0 ? _b : false; };
