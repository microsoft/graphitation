...
async function benchmarkWrites(queryKey) {
    const suite = new nice_benchmark_1.default(`${queryKey} - Write Operations`, {
...
async function benchmarkReads(queryKey) {
    const suite = new nice_benchmark_1.default(`${queryKey} - Read Operations`, {
        confidenceLevel: finalConfig.confidenceLevel
    });
    // Pre-populate cache
    const cache = createCache();
    const query = queries[queryKey];
    const testData = Array.from({ length: finalConfig.operationsPerIteration }, (_, i) => createTestData(queryKey, i));
    // Populate cache
...
async function benchmarkEmptyReads(queryKey) {
    const suite = new nice_benchmark_1.default(`${queryKey} - Empty Cache Reads (Cache Miss)`, {
        confidenceLevel: finalConfig.confidenceLevel
    });
...
async function benchmarkCacheMiss(queryKey) {
    const suite = new nice_benchmark_1.default(`${queryKey} - Cache Miss Operations`, {
        confidenceLevel: finalConfig.confidenceLevel
    });
...
async function benchmarkCacheHit(queryKey) {
    const suite = new nice_benchmark_1.default(`${queryKey} - Cache Hit Operations`, {
        confidenceLevel: finalConfig.confidenceLevel
    });
...
async function benchmarkMultipleObservers(queryKey) {
    const suite = new nice_benchmark_1.default(`${queryKey} - Multiple Observers`, {
        confidenceLevel: finalConfig.confidenceLevel
    });
...
async function benchmarkUpdates(queryKey) {
    const suite = new nice_benchmark_1.default(`${queryKey} - Update Operations`, {
        confidenceLevel: finalConfig.confidenceLevel
    });
