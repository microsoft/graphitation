...
const DIRECTIVE_WHITELIST = new Set([
...
class RelayParser {
  ...
  transform() {
    const nodes = [];
    const entries = new Map(); // Construct a mapping of name to definition ast + variable definitions.
  ...
  _buildArgumentDefinitions(definition) {
  ...
  _buildFragmentArgumentDefinitions(fragment) {
    ...
    const variableDirective = variableDirectives[0]; // work, below accesses all report arguments could still be null/undefined.
  ...
  _buildOperationArgumentDefinitions(operation) {
...
class GraphQLDefinitionParser {
  ...
  _recordAndVerifyVariableReference(variable, name, usedAsType) {
  ...
  _validateDirectivesLocation(directives, allowedLocaction) {
  ...
  _getLocationFromOperation(definition) {
  ...
  _transformOperation(definition) {
  ...
  _transformSelections(selectionSet, parentType, parentTypeAST) {
  ...
  _transformInlineFragment(fragment, parentType, parentTypeAST) {
    const schema = this._schema;
    let typeCondition =
  ...
  _transformFragmentSpread(fragmentSpread, parentType, parentTypeAST) {
  ...
  _transformField(field, parentType) {
    const schema = this._schema;
    const name = getName(field);
  ...
  _transformHandle(fieldName, fieldArgs, clientFieldDirectives) {
    let handles = null;
  ...
  _transformDirectives(directives, location) {
  ...
  _transformArguments(args, argumentDefinitions, field, directiveName) {
    return args.map((arg) => {
      const argName = getName(arg);
  ...
  _splitConditions(mixedDirectives) {
  ...
  _transformVariable(ast, usedAsType) {
...
function transformNonNullLiteral(schema, ast, type, transformVariable) {
  ...
  if (schema.isList(nullableType)) {
    ...
    if (value == null) {
      const suggestions = schema.getEnumValues(enumType); // parseLiteral() should return a non-null JavaScript value
...
function applyConditions(conditions, selections) {
  ...
  conditions.forEach((condition) => {
    nextSelections = [{ ...condition, selections: nextSelections }];
  });
